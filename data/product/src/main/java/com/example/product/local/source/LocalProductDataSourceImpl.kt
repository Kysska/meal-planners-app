...
class LocalProductDataSourceImpl(
    ...
    override fun getProductsInShopList(): Observable<List<Product>> {
        return productDao.getProductsInShopList()
            .map { products ->
                products.map { databaseMapper.reverseMap(it.product, it.date.date) }
            }
            .doOnError { throwable ->
                Timber.tag(TAG).e(throwable)
            }
    ...
    override fun getProductsInShopCart(): Observable<List<Product>> {
        return productDao.getProductsInShopCart()
            .map { products ->
                products.map { databaseMapper.reverseMap(it.product, it.date.date) }
            }
            .doOnError { throwable ->
    ...
    override fun moveProductInShopList(product: Product): Completable {
        return productDao.moveProductInShopList(
    ...
    override fun addProductInShopCart(product: Product): Completable {
        return productDao.addProductInShopCart(
            ProductInCartDbEntity(
                product = product.id,
                date = product.date
            )
